@foreach($ads as $ad)
<a href="{{ route('annonces.show', $ad->id) }}" class="bAd">
        <div class="card d-flex flex-row">
            <div class="card-header">
                @if($ad->photos->isNotEmpty())
                    <img class="rounded" src="{{ asset('thumbs/' . $ad->photos->first()->filename) }}" margin="auto" width="50%" alt="photo de couverture">
                @else
                    <img src="{{ asset('thumbs/question.jpg') }}" alt="pas de photo de couverture">
                @endif
            </div>
            <div class="card-body">
                <h4 class="card-title">{{ $ad->title }}</h4>
                <p class="card-text">{{ $ad->price }} euros </p>
                <p class="card-text">{{ $ad->category->name }}</p>
                <p class="card-text">
                    {{ $ad->commune_name . ' (' . $ad->commune_postal . ')'}}<br>
                    {{ $ad->created_at->calendar() }}
                </p>
            </div>
        </div>
    </a>
    <br>
@endforeach
<div class="d-flex">
    <div class="mx-auto">
        {{ $ads->links() }}
    </div>
</div>

